...
import { loginUser, registerTenant } from '../services/firebaseService';
...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const registeredTenant = await registerTenant({ ...newTenant, password });
      onLogin(registeredTenant);
        ...
        <div className="md:w-1/2 bg-slate-900 text-white p-8 md:p-12 flex flex-col justify-between relative overflow-hidden">
          ...
          <div className="relative z-10 space-y-4 mt-8">
            <div className="flex items-center space-x-3 text-slate-300">
              ...
              <span>Sincronización en la Nube (Firebase)</span>
            </div>
            <div className="flex items-center space-x-3 text-slate-300">
              <CheckCircle2 className="w-5 h-5 text-green-500" />
        ...
        <div className="md:w-1/2 p-8 md:p-12 flex flex-col justify-center bg-white">
          ...
          <form onSubmit={isRegistering ? handleRegister : handleLogin} className="space-y-5">
            ...
            <button 
              type="submit" 
              disabled={isLoading}
              className="w-full bg-orange-600 hover:bg-orange-700 disabled:bg-slate-400 text-white font-bold py-3 rounded-xl shadow-lg shadow-orange-600/20 transition-all flex items-center justify-center gap-2 group"
            >
