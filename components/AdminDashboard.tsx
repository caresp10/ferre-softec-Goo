...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onLogout }) => {
  ...
  useEffect(() => {
    ...
    if (storedTenants) {
      const parsed: Tenant[] = JSON.parse(storedTenants);
  ...
  const deleteTenant = (id: string) => {
    if (confirm('¿Estás seguro de eliminar esta organización y todos sus datos? Esta acción no se puede deshacer.')) {
      const updated = tenants.filter(t => t.id !== id);
      saveTenantsToStorage(updated);
      
      // Cleanup tenant data
      Object.keys(localStorage).forEach(key => {
        if (key.startsWith(`tenant_${id}`)) {
  ...
  const handleTenantSubmit = (e: React.FormEvent) => {
    ...
    const emailExists = tenants.some(t => 
    ...
    if (editingTenant) {
      const updated = tenants.map(t => 
        t.id === editingTenant.id ? { ...t, ...formData } as Tenant : t
      );
      saveTenantsToStorage(updated);
    } else {
      ...
      saveTenantsToStorage([...tenants, newTenant]);
    }
                  ...
                  {filteredTenants.map(tenant => (
                    <tr key={tenant.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-6 py-4 text-right space-x-2">
                        ...
                        <button onClick={() => toggleStatus(tenant.id)} className="text-slate-400 hover:text-orange-600"><Ban className="w-4 h-4" /></button>
